...
function estate_setup() {
	/*
		...
		* If you're building a theme based on estate, use a find and replace
		* to change 'estate' to the name of your theme in all the template files.
		*/
	...
	register_nav_menus(
		array(
			'menu-1' => esc_html__( 'Primary', 'estate' ),
		)
	);
 ...
 */
function estate_content_width() {
	$GLOBALS['content_width'] = apply_filters( 'estate_content_width', 640 );
}
add_action( 'after_setup_theme', 'estate_content_width', 0 );
...
function estate_widgets_init() {
	register_sidebar(
		array(
			'name'          => esc_html__( 'Sidebar', 'estate' ),
			'id'            => 'sidebar-1',
			'description'   => esc_html__( 'Add widgets here.', 'estate' ),
...
function estate_scripts() {
	wp_enqueue_style( 'estate-style', get_stylesheet_uri(), array(), _S_VERSION );
	wp_style_add_data( 'estate-style', 'rtl', 'replace' );

	wp_enqueue_script( 'estate-navigation', get_template_directory_uri() . '/js/navigation.js', array(), _S_VERSION, true );
...
function custom_setup_theme() {
	...
	// for($i=0;$i<10;$i++) {
	// 	$new_page = array(
	// 		'post_type'     => 'rental', 				// Post Type Slug eg: 'page', 'post'
