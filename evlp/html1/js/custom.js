...
let makoTargets = document.querySelectorAll('.pamako');
for (var i = 0; i < makoTargets.length; i++) {
	makoTargets[i].parentElement.style.overflow = "hidden";
	let maskClassToggle = new ScrollMagic.Scene({
		triggerElement: makoTargets[i],
		//reverse: false
	}).triggerHook('0.95').setClassToggle(makoTargets[i], 'on').addTo(controller);
}
let pongTargets = document.querySelectorAll('.paong');
for (var i = 0; i < pongTargets.length; i++) {
	let maskClassToggle = new ScrollMagic.Scene({
		triggerElement: pongTargets[i],
		//reverse: false
	}).triggerHook('0.95').setClassToggle(pongTargets[i], 'on').addTo(controller);
}
/*-- Magic animation end --*/
