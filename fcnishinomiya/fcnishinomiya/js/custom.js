...
/*----- Submenu Dropdown _START---------*/
$(function() { // Dropdown toggle
	$('.pdropdown_btn').click(function() { $(this).closest('.pdropdown') .find('.pdropdown_ul').slideToggle();});
	
	$(document).click(function(e) 
	{
		var target = e.target; 
		if (!$(target).is('.pdropdown_btn') && !$(target).parents().is('.pdropdown_btn')) 
			//{ $('.pdropdown_ul').hide(); }
		{ $('.pdropdown_ul').slideUp(); }
	});
});
/*----- Submenu Dropdown _END---------*/
...
$(".headmenu_big").click(function() {
	if($(window).width() < 768) {
		if($(this).hasClass("pst_open")) {
			$(this).removeClass("pst_open");
			$(this).find(".headmenu_subul").slideUp();
		} else {
			$(this).addClass("pst_open");
			$(this).find(".headmenu_subul").slideDown();
		}
	}
});
...
let pinTargets0 = document.querySelectorAll('.pinpos');
for (var i = 0; i < pinTargets0.length; i++) {
	let pinClassToggle0 = new ScrollMagic.Scene({
		triggerElement: pinTargets0[i],
		//reverse: false
	}).triggerHook('0.8').setClassToggle(pinTargets0[i], 'run_ani').addTo(controller);
}
/*-- Magic animation end --*/
...
/*--carousel--*/
$(function() {
	if($('.tpfv_slide').length>0) {
		$('.tpfv_slide').owlCarousel({
			loop:true,
			autoplay:true,
...
$(function() {
	if($('.tpmatch_slide').length>0) {
		$('.tpmatch_slide').owlCarousel({
			loop:false,
			autoplay:false,
...
$(".ptab_btn")
	.click(function () {
		$(this).closest('.ptab').find('.ptab_btn')
			...
			.eq($(this).index())
			.addClass("pst_active");
			$(this).closest('.ptab').find('.ptab_item').hide().eq($(this).index()).fadeIn();
			console.log("few");
