...
$(".js_hamburger").click(function() {
	...
	$(".hamburger_text").toggleClass("pst_open");
...
let darkTargets = document.querySelectorAll('.pindark');
for (var i = 0; i < darkTargets.length; i++) {
    let darkClassToggle = new ScrollMagic.Scene({
        triggerElement: darkTargets[i],
        //reverse: false
    }).triggerHook('0.5').setClassToggle(darkTargets[i], 'run_dark').addTo(controller);
}
...
for (var i = 0; i < pinTargets.length; i++) {
	let pinClassToggle = new ScrollMagic.Scene({
		...
		//reverse: false
	}).triggerHook('0.75').setClassToggle(pinTargets[i], 'run_ani').addTo(controller);
}
/*-- Magic animation end --*/
