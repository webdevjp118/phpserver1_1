...
$(function() {
	$(window).load(function() {
		if($('#history_id').length > 0) {
			if($('#history_id').val()) {
				if ($.cookie($('#history_id').val()) != 'on') {
					$.cookie($('#history_id').val(), 'on', { expires: 600 });
					$('#first_visit').css({'display':'flex'});
				} else {
					$('#first_visit').remove();	
				}
			} else {
...
$(function(){
	$(".dial_close").click(function () {
		$(".dial_modal").css({"display": "none"});
	});
	$(".open_dial_modal").click(function () {
		$(".dial_modal").css({"display": "flex"});
	});
});
/*--dial modal end--*/
...
$(function(){
	...
	function hideLoading() {
		...
		$('.top_fv, .pg_fv, .fvani_pos').addClass('run_ani');
	}
});
/*-- SITE_LOADING_END --*/
