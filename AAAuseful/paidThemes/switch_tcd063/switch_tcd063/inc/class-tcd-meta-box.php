...
class TCD_Meta_Box {
    ...
    if ( isset( $field['max'] ) ) $attributes[] = 'max="' . esc_attr( $field['max'] ) . '"';
		?>
    <input type="number" class="tiny-text" id="<?php echo esc_attr( $field['id'] ); ?>" name="<?php echo esc_attr( $field['id'] ); ?>" value="<?php echo esc_attr( $value ); ?>" <?php echo implode( ' ', $attributes ); ?>> <?php if ( isset( $field['unit'] ) ) { echo esc_html( $field['unit'] ); } ?>
		<?php
		echo $field['after_field'];
	}
	...
	public function render_textarea( $field, $value ) {
		...
		?>
    <textarea class="large-text" id="<?php echo esc_attr( $field['id'] ); ?>" name="<?php echo esc_attr( $field['id'] ); ?>"<?php if ( isset( $field['rows'] ) ) { echo ' rows="' . $field['rows'] . '"'; } ?>><?php echo esc_textarea( $value ); ?></textarea>
		<?php
		echo $field['after_field'];
	}
    ...
    // Create a template
    $clone = '<tr class="a-table__row repeater-item">';

    foreach ( array_keys( $field['header'] ) as $key ) {
      $clone .= '<td>';
      $clone .= '<textarea name=' . esc_attr( $field['id'] ) . '[' . esc_attr( $key ) . '][]" class="widefat"></textarea>';
      $clone .= '</td>';
    }
    $clone .= '<td class="col-delete u-center">';
    ...
    if ( isset( $value['header'][0] ) ) {
        ...
        foreach ( array_keys( $field['header'] ) as $key ) {
          ...
          echo '<td>';
          echo '<textarea name="' . esc_attr( $field['id'] ) . '[' . esc_attr( $key ) . '][]" class="widefat">' . esc_textarea( $data ) . '</textarea>';

          echo '</td>';

        }

			  echo '<td class="col-delete u-center"><a href="#" class="button button-secondary button-delete-row">' . __( 'Delete', 'tcd-w' ) . '</a></td>';
			  echo '</tr>' . "\n";
		  }
	  } else {
		  echo $clone . "\n";
	...
	/**
	 * Render a video field
	 */
	public function render_video( $field, $value ) {
    ...
    <div class="image_box cf">
      <div class="cf cf_media_field hide-if-no-js <?php echo esc_attr( $field['id'] ); ?>">
		  	<input type="hidden" value="0" id="<?php echo esc_attr( $field['id'] ); ?>" name="<?php echo esc_attr( $field['id'] ); ?>" class="cf_media_id">
		  	<div class="preview_field preview_field_video">
		    		<?php if ( $value ) : ?>
		    		<h5><?php _e( 'Uploaded MP4 file', 'tcd-w' ); ?></h5>
        		<p><?php echo esc_html( wp_get_attachment_url( $value ) ); ?></p>
		    		<?php endif; ?>
		    </div>
      	<div class="button_area">
      		<input type="button" value="<?php _e( 'Select MP4 file', 'tcd-w' ); ?>" class="cfmf-select-video button">
      		<input type="button<?php if ( ! $value ) { echo ' hidden'; } ?>" value="<?php _e( 'Remove MP4 file', 'tcd-w' ); ?>" class="cfmf-delete-video button hidden">
      	</div>
      </div>
    </div>
	<?php
