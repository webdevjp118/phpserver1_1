...
function artist_setup() {
	...
	register_nav_menus(
		array(
			'menu-1' => esc_html__( 'Primary', 'artist' ),
		)
	);
 ...
 */
function artist_content_width() {
	$GLOBALS['content_width'] = apply_filters( 'artist_content_width', 640 );
}
add_action( 'after_setup_theme', 'artist_content_width', 0 );
...
function artist_widgets_init() {
	register_sidebar(
		array(
			'name'          => esc_html__( 'Sidebar', 'artist' ),
			'id'            => 'sidebar-1',
			'description'   => esc_html__( 'Add widgets here.', 'artist' ),
...
function artist_scripts() {
	wp_enqueue_style( 'artist-style', get_stylesheet_uri(), array(), _S_VERSION );
	wp_style_add_data( 'artist-style', 'rtl', 'replace' );

	wp_enqueue_script( 'artist-navigation', get_template_directory_uri() . '/js/navigation.js', array(), _S_VERSION, true );
...
function custom_setup_theme() {
	...
	// for($i=0;$i<10;$i++) {
	// 	$new_page = array(
	// 		'post_type'     => 'info', 				// Post Type Slug eg: 'page', 'post'
	// 		'post_title'    => 'タイトルテキストがあります。タイトルテキストがあります。タイトルテキストがあります。',	// Title of the Content
	// 		'post_status'   => 'publish',			// Post Status
	// 		'post_name'     => 'info'.$i			// Slug of the Post
	// 	);
